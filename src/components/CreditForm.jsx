...
render() {
    ...
    <form onSubmit={this.handleSubmit}>
      <div class="form-group">
        <label>Select the purpose:</label>
      ...
      <div class="form-group">
        <label>How much funding do you need : (total in euros)</label>
        <input type="text" name="walletAddress" onChange={this.handleChange} class="form-control" required/>
      </div>
      <div class="form-group">
        <label>How often would you like to receive payment tranches? monthly, every 3 month, every 6 month, once a year</label>
        <input type="text" name="password" onChange={this.handleChange} class="form-control" required/>
      </div>
      <Link to ='/KycVerify' ><button>Next Step</button></Link>
    </form>
  );
}
}
